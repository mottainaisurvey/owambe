...
// ─── TEMP: AC-F1-PROD-02 probe calibration — create T1 probe experience + slot ──────────────
// OWB-F1-NEW-IMPLEMENTATION-01 AC-F1-PROD-02 production probe calibration endpoint.
// Creates a minimal T1 probe experience + time slot for production smoke probe use.
// Returns the experience ID and slot ID.
// MUST be removed after production probe is complete.
adminRouter.post('/temp/probe-seed', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!operatorRecord) {
      ...
      const createdUser = await prisma.user.create({
        data: {
          ...
          availableModes: ['EXPERIENCES'] as any,
          activeMode: 'EXPERIENCES' as any,
          operator: {
            create: {
    ...
    // Find or create a far-future time slot (2030-07-04)
    const slotStart = new Date('2030-07-04T15:00:00.000Z');
    const slotEnd = new Date('2030-07-04T18:00:00.000Z');
    let slot = await prisma.experienceSlot.findFirst({
      where: { experienceId: experience.id, startTime: slotStart },
    });
    if (!slot) {
